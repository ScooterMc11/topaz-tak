//! 5x5 black-stack opening-book generator (Board5 port of [`crate::openings`]).
//!
//! Same rules as the 6x6 book, adapted to 5x5 geometry, with ONE deliberate change: plies 5-6 use a
//! maximum of **3** orthogonal (Manhattan) steps from an own-colour piece, vs 4 at 6x6.
//!
//! Emits a **TPS book** (one position per line, for racetrack `--book-format tps`) and a parallel
//! **PTN book**. TPS rather than a move list because racetrack arbitrates with stock `tiltak`
//! (standard rules), which rejects the forced `2xx` double placement; baking the opening into a TPS
//! start position (at `move_num >= 2`) sidesteps it — Topaz's black-stack rule fires only at
//! `move_num == 1`. See auto-memory `racetrack-tps-book-requirement`.
//!
//! Opening structure (6 plies, flats only):
//!   - ply 1: White's forced `2xx` double-black-stack
//!   - ply 2: Black's single swapped flat
//!   - plies 1-2 from 5 archetypes: diagonal corners, adjacent corners, hug, gap-hug, center stack
//!   - plies 3-4: flats restricted to the interior 3x3 (b2..d4; no edge)
//!   - plies 5-6: flats within `MAX_STEPS` orthogonal steps of an own-colour piece. The opening swap
//!     means White's ply-1 double placement is BLACK and Black's ply-2 flat is (swapped) WHITE.
//!
//! D4-equivalent positions (8 rotations/reflections) are de-duplicated.
...
/// 3 for 5x5 (vs 4 at 6x6) — the requested rule change.
const MAX_STEPS: i32 = 3;

/// Ply 1-2 archetypes (White double-stack square, Black flat square), in 5x5 coordinates. Symmetry
/// de-duplication on the final 6-ply position collapses equivalents.
const DIAGONAL: &[(&str, &str)] = &[("a1", "e5"), ("e5", "a1"), ("a5", "e1"), ("e1", "a5")];
...
/// 5x5 has a single true center.
const CENTERS: [&str; 1] = ["c3"];
const CORNERS: [&str; 4] = ["a1", "a5", "e1", "e5"];
...
/// Black's first-reply square to White's double-black-stack on a corner (PlayTak GemBot use). 5x5
/// port of [`crate::openings::pick_black_reply`]: weights = `[diagonal, adjacent, hug, gap_hug]`,
/// returns `None` for a non-corner White opening (caller falls back to search). Self-seeds its RNG.
...
impl Default for OpeningsConfig {
    fn default() -> Self {
        Self {
            count: 1000,
            tps_path: "5s_black_stack_openings.tps".to_string(),
            ptn_path: "5s_black_stack_openings.ptn".to_string(),
            standard: false,
        }
    }
}
...
/// All 25 squares, as "<file><rank>" strings.
...
/// The interior 3x3 (b2..d4): squares that are not on the board edge.
...
fn apply_ply(board: &mut Board5, sq: &str, double: bool) {
    let ptn = if double {
        format!("2{sq}")
    } else {
        sq.to_string()
...
fn build_opening<R: RngCore>(
    ...
    let mut board = Board5::new(); // komi 0
    let mut occupied: Vec<String> = Vec::with_capacity(6);
    let mut ptn: Vec<String> = Vec::with_capacity(6);

    // Reference squares by piece colour for the ply 5-6 proximity constraint. The opening swap means
    // White's ply-1 placement is BLACK and Black's ply-2 flat is WHITE.
    let mut white_refs: Vec<String> = Vec::new();
    let mut black_refs: Vec<String> = Vec::new();

    // ply 1: White's opening -> a black piece at w1. Standard mode: a single swapped flat.
...
/// equal the active archetype count). Archetypes are interleaved by smooth weighted round-robin.
/// Returns `(tps, ptn, archetype_index)` per opening.
    ...
    while out.len() < count {
        ...
        while !added {
            if attempts >= max_attempts {
                eprintln!(
                    "openings5: stopped after {attempts} attempts with {} unique positions (target {})",
                    out.len(),
                    count
                );
                return out;
...
pub fn run(cfg: OpeningsConfig) -> io::Result<usize> {
    ...
    for (tps, _, _) in &openings {
        if Board5::try_from_tps(tps).is_err() {
    ...
    println!(
        "openings5: wrote {} unique positions -> {} (tps), {} (ptn)",
        openings.len(),
        cfg.tps_path,
        cfg.ptn_path
    );
...
mod test {
    ...
    fn interior_squares_exclude_edges() {
        ...
        assert_eq!(interior.len(), 9); // inner 3x3 on 5x5
        for sq in &interior {
            let file = sq.chars().next().unwrap();
            let rank: usize = sq[1..].parse().unwrap();
            assert!(('b'..='d').contains(&file) && (2..=4).contains(&rank), "{sq} is on an edge");
        }
    }
    ...
    #[test]
    fn plies_5_6_within_three_steps_of_same_colour() {
        let mut rng = new_rng();
        let all_sq = all_squares();
        let interior_sq = interior_squares();
        for _ in 0..300 {
    ...
    fn generated_openings_are_unique_and_valid() {
        let cfg = OpeningsConfig {
            count: 50,
            tps_path: std::env::temp_dir().join("topaz5_test_openings.tps").to_string_lossy().into_owned(),
            ptn_path: std::env::temp_dir().join("topaz5_test_openings.ptn").to_string_lossy().into_owned(),
            standard: false,
        };
