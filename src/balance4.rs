//! Self-play balance assessment for 4x4 (Board4/NNUE4 port of [`crate::balance5`]).
//!
...
//! (komi 0, black-stack net + black-stack book) against **standard** 4x4 (komi 0, standard book, same
//! black-stack net — standard rules are too imbalanced to train a usable native net). A result near
//! 50/50 is balanced; far from it is not.
//!
...
//!
//! NOTE: the 4x4 opening ceiling is small (207 black-stack book positions; see `openings4`), so
//! `--archetypes`/`--book` runs cap the game count at the number of distinct openings.

use crate::board::{Board4, TakBoard};
use crate::datagen::new_rng;
use crate::datagen4::{do_random_flat_move, Tiltak};
use crate::eval::NNUE4;
...
pub struct BalanceConfig {
    ...
    /// are ignored. See `topaz openings4`.
    ...
    pub ptn_limit: usize,
    pub tt_size: usize,
    /// If set, drive this tiltak TEI binary (self-play) instead of the embedded NNUE4 net — a
...
fn play_one(
    ...
    let mut board = match start {
        Some(tps) => Board4::try_from_tps(tps)
            .expect("balance book contained an invalid tps")
            .with_komi(cfg.komi),
        None => {
            let mut board = Board4::new().with_komi(cfg.komi);

            // Optionally pin White's opening double-black-stack to a (random) corner.
            if cfg.corner_open {
                const CORNERS: [&str; 4] = ["2a1", "2d1", "2a4", "2d4"];
                let mv =
                    GameMove::try_from_ptn_m(CORNERS[(rng.next_u32() % 4) as usize], 4, Color::White)
    ...
    // Best play to completion — driven by tiltak (TEI) if provided, else Topaz NNUE4 search.
    if let Some(tiltak) = tiltak {
        let _ = tiltak.new_game(4);
...
fn moves_to_ptn(moves: &[GameMove], start_move_num: usize, white_first: bool) -> String {
    ...
    if !white_first && i < moves.len() {
        s.push_str(&format!("{num}. -- {} ", moves[i].to_ptn::<Board4>()));
        i += 1;
        num += 1;
    }
    while i < moves.len() {
        let w = moves[i].to_ptn::<Board4>();
        i += 1;
        if i < moves.len() {
            let b = moves[i].to_ptn::<Board4>();
            i += 1;
...
pub fn run(cfg: BalanceConfig) -> BalanceResult {
    ...
    let labeled = cfg.archetype_breakdown.then(|| match &cfg.archetype_weights {
        Some(w) => crate::openings4::generate_weighted(cfg.num_games, cfg.standard_book, w),
        None => crate::openings4::generate(cfg.num_games, cfg.standard_book),
    });
    let num_buckets = if cfg.archetype_breakdown {
        crate::openings4::num_archetypes(cfg.standard_book)
    } else {
        1
    };
    ...
    if cfg.archetype_breakdown {
        println!(
            "balance4: {} generated {} openings (per-archetype breakdown{}), {} threads, {} nodes/move (half-komi {})",
            total_games,
        ...
        println!(
            "balance4: {} book positions from {} (1 deterministic game each), {} threads, {} nodes/move (half-komi {})",
            b.len(), cfg.book_path.as_deref().unwrap_or(""), cfg.threads, cfg.max_nodes, cfg.komi,
        );
    } else {
        println!(
            "balance4: {} games, {} threads, {} nodes/move, {} random flat plies{} (half-komi {})",
    ...
    for t in 0..threads {
        ...
        handles.push(thread::spawn(move || {
            ...
            let mut tiltak = cfg
                .tiltak_path
                .as_ref()
                .map(|p| Tiltak::spawn(p, 4, cfg.komi).expect("failed to spawn tiltak"));
    ...
    if cfg.archetype_breakdown {
        ...
        for (i, r) in per.iter().enumerate() {
            println!(
                "\n[{}]  ({} games, {:.1}% of book)",
                crate::openings4::archetype_name(i),
                r.games,
...
mod test {
    ...
    #[test]
    fn balance4_accounts_for_all_games() {
