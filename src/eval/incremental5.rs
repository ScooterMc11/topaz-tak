//! Size-5 (5x5) NNUE mapper. This is a self-contained duplicate of the 6x6
//! `incremental.rs`, re-parameterised for a 25-square board with 22 reserve
//! pieces per side (21 flats + 1 cap). It intentionally shares **nothing** with
//! the 6x6 module so that 5x5 work cannot perturb the proven Board6 net.
//!
//! Feature layout (must stay byte-identical with the 5x5 trainer's feature map):
//!   - `SQUARE_INPUTS = 25 * (6 + 2*STACK_DEPTH) = 650`; perspective split at 325.
//!   - side-to-move:  650, 651
//!   - reserve band A: 658..=680  (`SQUARE_INPUTS + 8 + reserves`)
//!   - reserve band B: 694..=716  (`SQUARE_INPUTS + 44 + reserves`)
//!   - `NUM_INPUTS = 738` (`SQUARE_INPUTS + 88`, mirroring the 6x6 padding).
//! The offsets deliberately mirror the 6x6 formula so only `SQUARE_INPUTS`
//! differs; this keeps the trainer port mechanical.
...
/// Number of board squares (5x5).
const SQUARES: usize = 25;
/// Reserve pieces per side that the iterator counts down from (21 flats + 1 cap).
const RESERVE_INIT: usize = 22;
/// Max distinct (square, depth) entries: every piece of both sides = 2 * 22.
const DATA_LEN: usize = 44;

pub static NNUE5: Network = unsafe {
    let bytes = include_bytes!("../quantised5.bin");
...
#[derive(Clone, Copy)]
pub struct TakSimple5 {}

impl TakSimple5 {
    pub const SQUARE_INPUTS: usize = SQUARES * (6 + 2 * STACK_DEPTH); // 650
    // Squares + Side + Reserves
    pub const NUM_INPUTS: usize = TakSimple5::SQUARE_INPUTS + 8 + 80; // Pad to 738
    /// Perspective split: own pieces in [0, SPLIT), opponent in [SPLIT, SQUARE_INPUTS).
    const SPLIT: usize = TakSimple5::SQUARE_INPUTS / 2; // 325
    /// Base of the second reserve one-hot band.
    const RES_B_BASE: usize = TakSimple5::SQUARE_INPUTS + 44; // 694
...
impl NNUE5 {
    pub fn incremental_eval(&mut self, takboard: BoardData) -> i32 {
        ...
        // Output
        let eval = NNUE5.evaluate(&ours.vec, &theirs.vec, ours.state.clone().into_iter());
    ...
    pub(crate) fn manual_eval(takboard: BoardData) -> i32 {
        let (ours, theirs) = build_features(takboard);
        let ours = Incremental::fresh_new(&NNUE5, ours);
        let theirs = Incremental::fresh_new(&NNUE5, theirs);
        let eval = NNUE5.evaluate(&ours.vec, &theirs.vec, ours.state.clone().into_iter());
        eval
    }
}

impl Default for NNUE5 {
    fn default() -> Self {
        Self {
            white: (
                Incremental::fresh_empty(&NNUE5),
                Incremental::fresh_empty(&NNUE5),
            ),
            black: (
                Incremental::fresh_empty(&NNUE5),
...
pub struct Network {
    ...
    feature_weights: [Accumulator; TakSimple5::NUM_INPUTS],
    ...
    pqst: [i16; TakSimple5::NUM_INPUTS],
    /// Scalar output bias.
    output_bias: i16,
}
            ...
            .enumerate()
        {
            ...
            if d != 0 {
                ...
                while sub != 0 {
                    ...
                    acc.remove_feature(idx * 64 + bit_idx as usize, &NNUE5);
                }
                while add != 0 {
                    let bit_idx = pop_lowest(&mut add);
                    acc.add_feature(idx * 64 + bit_idx as usize, &NNUE5);
                }
            }
        }
    }
