#!/usr/bin/env bash
#
# 4x4 strength test via racetrack, on the black-stack TPS opening book (komi 0).
#
# Default mode: a fixed round-robin **Elo run, topaz (NNUE4) vs tiltak** — measures how close the new
# 4x4 net is to the strongest 4x4 engine. Each opening is played twice (both colours), so games =
# 2 x openings.
#
# net-vs-net regression SPRT (for iterating): set ENGINE_B=/path/to/other/topaz and SPRT=1. Build the
# "other" topaz by copying the comparison net to src/quantised4.bin, `touch src/eval/incremental4.rs`,
# `cargo build --release`, and saving the binary under a distinct name first.
#
...
#
# NOTE: the 4x4 opening book is EXHAUSTIVE and small (207 black-stack / 146 standard unique D4
# positions). So games cap at 2x that (414 / 292) — coarser SPRT than 5x5's 4000. The book is
# deterministic, so it is (re)generated only when missing.
set -euo pipefail
...
PTNOUT=${PTNOUT:-sprt4-games.ptn}

# STANDARD=1 switches to the standard-Tak opening book (single-flat, 3 archetypes); default is the
# black-stack book (forced 2xx, 4 archetypes). This sets the default BOOK and the openings4 flag used
# to (re)generate it, so net-vs-net SPRT for the standard net auto-uses the right book.
STANDARD=${STANDARD:-0}
if [ "$STANDARD" = "1" ]; then
  OPENING_FLAG="--standard"; VARIANT="standard"; BOOK_DEFAULT="4s_standard_openings.tps"
else
  OPENING_FLAG="";           VARIANT="black-stack"; BOOK_DEFAULT="4s_black_stack_openings.tps"
fi
...
# (Re)generate the opening book only if missing — openings4 is exhaustive + deterministic, so the
# default count (207) caps at the variant's ceiling (207 black-stack / 146 standard).
if [ ! -s "$BOOK" ]; then
  say "Generating $VARIANT book -> $BOOK"
  ./target/release/topaz openings4 $OPENING_FLAG -t "$BOOK" -p "${BOOK%.tps}.ptn"
fi
...
if [ "$SPRT" = "1" ] && [ -n "$ENGINE_B" ]; then
  ...
  "$RACETRACK" -s 4 --komi "$KOMI" -c "$CONCURRENCY" -g "$GAMES" \
